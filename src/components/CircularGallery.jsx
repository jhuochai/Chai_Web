...
class App {
  ...
  createVisibilityObserver() {
    // Continuous WebGL rendering is wasted work (and battery) once the
    // gallery scrolls out of view, so the render loop pauses off-screen
    // and picks back up the moment it re-enters the viewport.
    this.observer = new IntersectionObserver(
      ([entry]) => {
        const wasVisible = this.isVisible;
        this.isVisible = entry.isIntersecting;
        if (this.isVisible && !wasVisible) this.update();
      },
      { threshold: 0 }
    );
  ...
  addEventListeners() {
    ...
    // Scoped to the gallery container (not window) so this component can
    // live inside a normal page without hijacking clicks/scroll elsewhere.
...
export default function CircularGallery({
  ...
  textColor = '#ffffff',
  borderRadius = 0.05,
  font = 'bold 28px "Fraunces Variable"',
  scrollSpeed = 2,
  scrollEase = 0.02,
  ...
  useEffect(() => {
    ...
    const ready = document.fonts?.load ? document.fonts.load(font).catch(() => {}) : Promise.resolve();
